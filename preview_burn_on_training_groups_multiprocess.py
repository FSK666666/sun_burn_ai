...
# 多进程时限制每个子进程内部的 BLAS/OpenMP 进程数，
# 防止“进程数 × 库内部进程数”造成线程过度竞争。
...
TRAIN_ROOT = r"C:\Users\17874\Documents\python\datasets\burn_recovery\test"
...
# 是否启用多进程。仅用于自动写入模式；Matplotlib 交互预览仍在主进程执行。
ENABLE_MULTIPROCESSING = True

# 最大进程数。R7 8745H/HS 建议先用 6，再测试 4、8 的速度。
# None 时自动使用 min(8, CPU逻辑核心数)。
MAX_WORKERS = 5

# 多进程时关闭每个进程中的 OpenCV 内部线程，避免过度嵌套。
...
def _process_one_group_auto_write(
    ...
    """单个子进程负责一组样本的读取、灼烧生成和保存。"""
    if DISABLE_OPENCV_INTERNAL_THREADS:
        cv2.setNumThreads(1)
...
def auto_write_burn_artifacts(
    ...
    多进程模式下：
    - 主进程预先为每个样本分配唯一随机 seed；
    - 子进程独立读取图像、生成灼烧并写入各自样本目录；
    - 只向子进程传递路径和 seed，避免在进程间复制大数组；
    ...
    # 预先生成 seed，避免多个进程共享同一个随机数生成器。
    seeds = rng.integers(0, 10**9, size=total, dtype=np.int64)

    written = 0
    failed = 0

    if not ENABLE_MULTIPROCESSING or total == 1:
        print("Auto write mode: single-process.")
    ...
    print(f"Auto write mode: multiprocessing, max_workers={max_workers}.")

    # Windows 使用 spawn，兼容性更好；Linux/macOS 也可正常运行。
...
def main():
    if DISABLE_OPENCV_INTERNAL_THREADS and ENABLE_MULTIPROCESSING:
        cv2.setNumThreads(1)
...
if __name__ == "__main__":
    # Windows 多进程必须保留此保护，打包为 exe 时也需要 freeze_support。
    mp.freeze_support()
    main()
