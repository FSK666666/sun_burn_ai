...
class DepthwiseSeparableConv(nn.Module):
    """Depthwise 3x3 + pointwise 1x1 convolution."""
...
class LiteMultiScaleBlock(nn.Module):
    """Lightweight local/context multiscale block."""
...
class BurnRecoveryNet(nn.Module):
    """Temporal-fusion dual-head lightweight U-Net-like network."""
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if x.shape[1] != self.in_frames:
            raise ValueError(f"Expected {self.in_frames} frames, got {x.shape[1]}")
...
def restore_future_frame(
    ...
    if threshold is None:
        ...
        restored = future_frame - (prob > threshold).to(correction.dtype) * correction
