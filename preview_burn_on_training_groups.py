...
# Automatic mode progress interval. Frequent console output slows large batches.
AUTO_WRITE_PRINT_EVERY = 100

# Parallel workers for automatic mode. Set to 1 for strictly single-process runs.
...
_COORDINATE_GRID_CACHE: dict[tuple[int, int], tuple[np.ndarray, np.ndarray]] = {}
_EDGE_WEIGHT_CACHE: dict[tuple[int, int, float, float], np.ndarray] = {}
...
def list_group_image_paths(group_dir: Path) -> list[Path]:
    paths = sorted(
        [
    ...
    return paths[:EXPECTED_FRAMES]


def load_group_shape(group_dir: Path) -> tuple[int, int]:
    paths = list_group_image_paths(group_dir)
...
def _coordinate_grid(h, w):
    key = (int(h), int(w))
    if key not in _COORDINATE_GRID_CACHE:
        _COORDINATE_GRID_CACHE[key] = np.mgrid[0:h, 0:w]
    return _COORDINATE_GRID_CACHE[key]


def _distance_to_edge_map(h, w):
    yy, xx = _coordinate_grid(h, w)
...
def _edge_weight_map(h, w, edge_falloff=25, min_weight=0.25):
    key = (int(h), int(w), float(edge_falloff), float(min_weight))
...
def generate_active_burn_map(h: int, w: int, seed: int) -> np.ndarray:
    size_scale = get_burn_size_scale()
    ...
    return np.where(active_mask, burn_map, 0.0).astype(np.float32)
...
def apply_burn_to_group(
    ...
    burn_map = generate_active_burn_map(h, w, seed)
    active_mask = burn_map > BURN_ACTIVE_THRESHOLD
...
def _auto_write_one_burn_artifact(task):
    group_index, total, group_dir_text, seed = task
    group_dir = Path(group_dir_text)

    try:
        cv2.setNumThreads(1)
        h, w = load_group_shape(group_dir)
        burn_map = generate_active_burn_map(h, w, seed)
        ...
        return {
            ...
            "trace_name": saved_paths["trace_npy"].name,
            "probability_name": saved_paths["probability_png"].name,
            "error": "",
        }
...
def _should_print_auto_progress(group_index: int, total: int) -> bool:
    return (
        group_index == 1
        or group_index == total
        or AUTO_WRITE_PRINT_EVERY <= 1
        or group_index % AUTO_WRITE_PRINT_EVERY == 0
    )


def _print_auto_write_result(result: dict):
    group_index = result["group_index"]
    total = result["total"]
    if result["ok"]:
        if _should_print_auto_progress(group_index, total):
            print(
                f"[{group_index}/{total}] saved {result['group_name']}: "
                f"{result['trace_name']}, "
                f"{result['probability_name']}"
            )
    else:
        print(
            f"[{group_index}/{total}] failed {result['group_name']}: "
            f"{result['error']}"
        )
...
def auto_write_burn_artifacts(
    ...
    written = 0
    failed = 0
    selected_dirs = group_dirs[:total]
    tasks = [
        (
            group_index,
            total,
        ...
        for group_index, group_dir in enumerate(selected_dirs, start=1)
    ]

    if AUTO_WRITE_WORKERS <= 1:
        for task in tasks:
            result = _auto_write_one_burn_artifact(task)
            if result["ok"]:
                written += 1
            else:
                failed += 1
            _print_auto_write_result(result)
    else:
        print(f"Using {AUTO_WRITE_WORKERS} worker processes for automatic writing.")
        with ProcessPoolExecutor(max_workers=AUTO_WRITE_WORKERS) as executor:
            for result in executor.map(_auto_write_one_burn_artifact, tasks, chunksize=8):
                if result["ok"]:
                    written += 1
                else:
                    failed += 1
